...
import { Bell, ChevronLeft, ChevronRight, Plus, Users, ClipboardList, Loader2, Search, Filter, X, Network, Database, Wifi } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 text-gray-800">
      ...
      <div className="fixed bottom-4 left-4 z-40">
        ...
        {isIpSimOpen && (
            <div className="absolute bottom-14 left-0 w-72 bg-white rounded-lg shadow-xl border border-gray-200 p-4 animate-in slide-in-from-bottom-2 max-h-[400px] overflow-y-auto">
                <div className="flex items-center justify-between mb-2">
                    <h4 className="text-xs font-bold text-gray-500 uppercase">Ağ Simülasyonu</h4>
                    {isFetchingIp ? (
                ...
                <div className="p-2 bg-yellow-50 border border-yellow-100 rounded mb-3">
                    <p className="text-[10px] text-yellow-800 leading-tight">
                        <strong>Not:</strong> Deploy edildiğinde yerel (192.168.x.x) IP'ler çalışmaz. Admin paneline <strong>Public IP</strong> adresinizi girmelisiniz.
                    </p>
                </div>
                ...
                <div className="space-y-2">
                    ...
                    {Object.entries(ipConfig.departmentIps).map(([ip, deptId]) => {
                     ...
                     <div className="h-px bg-gray-100 my-1"></div>
                     <button 
                        onClick={() => { setCurrentIp(''); setIsIpSimOpen(false); }}
                        className={`w-full text-left px-3 py-2 rounded text-xs flex justify-between items-center ${!currentIp ? 'bg-gray-200 text-gray-700 font-bold' : 'hover:bg-gray-50'}`}
                    >
                        <span>Misafir Modu</span>
                        <span className="opacity-50">(Erişim Yok)</span>
                    </button>
                </div>
            </div>
        )}
